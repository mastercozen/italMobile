...
//Constructor
   ...
   public function index(){
    ...
    public function ajax_view_stocks_online_by_id($id) {
     ...
     public function ajax_view_stocks_online_forecasted_by_id($id) {
        $this->load->model('Product_model');
        $data["result"] = $this->Product_model->getStocksOnlineForecastedById($id);
        echo json_encode($data);
    }
    ...
    public function update_product() {
        ...
        $this->form_validation->set_rules('main_ing', 'main_ing'); //VALIDATION OKAY
    ...
    public function update_product_image() {
        ...
        $config['upload_path'] = '<?php echo base_url() ?>../italUploads/uploads/itit';    //folder na pagssave ng pic
    ...
    public function inventory_in_walkin(){
     ...
     public function inventory_in_online_f(){
        if ($this->session->userdata('admin_logged_in')) {

            $this->load->model('Inventory_model');
            $data['result']=$this->Inventory_model->get_inventory_in_online_f();

            $this->load->view('administrator/includes/header');
            $this->load->view('administrator/includes/topbar');
            $this->load->view('administrator/includes/sidebar');
            $this->load->view('administrator/inventory-in-online-f-view',$data);
            $this->load->view('administrator/includes/footer');
        } 
        else {
           redirect('Administrator', 'refresh');
	...
	if ($this->form_validation->run() == FALSE) {
		...
		$pb->Alert("From Italiannis: ".strip_tags($this->input->post('message')));
